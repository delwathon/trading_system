...
class SignalGenerator:
    ...
    # def great_old_create_buy_signal(self, symbol_data: Dict, current_price: float, latest: pd.Series,
    ...
    # def great_old_create_sell_signal(self, symbol_data: Dict, current_price: float, latest: pd.Series,
    #                       volume_entry: Dict, confluence_zones: List[Dict],
    #                       tech_score: int, trend_score: int, stoch_score: int, structure_score: int) -> Dict:
    ...
    def create_buy_signal(self, symbol_data: Dict, current_price: float, latest: pd.Series,
                        volume_entry: Dict, confluence_zones: List[Dict],
                        tech_score: int, trend_score: int, stoch_score: int, structure_score: int) -> Dict:
        """TUNED: Create BUY signal using market structure for stop loss and take profit"""
        try:
            # Entry price logic for BUY
            entry_candidates = [current_price]
            ...
            support_zones = [zone for zone in confluence_zones
                           if zone['zone_type'] == 'support' and 
                           zone['price'] < current_price * 1.02 and
                           zone['price'] > current_price * 0.95]
            ...
            # stop_loss = min([zone['price'] for zone in confluence_zones if zone['zone_type'] == 'support' and zone['price'] < optimal_entry], default=optimal_entry * 0.97)
            # Find nearest resistance above entry for take profit
            take_profit_1 = min([zone['price'] for zone in confluence_zones if zone['zone_type'] == 'resistance' and zone['price'] > optimal_entry])
            # Optionally, set a further TP2 at next resistance or a fixed multiple
            take_profit_2 = min([zone['price'] for zone in confluence_zones if zone['zone_type'] == 'resistance' and zone['price'] > take_profit_1])

            # Fallback if no structure found
            if take_profit_1 >= optimal_entry:
                take_profit_1 = 0
            if take_profit_2 >= take_profit_1:
                take_profit_2 = 0
            # if stop_loss >= optimal_entry:
            #     stop_loss = optimal_entry * 0.97
            # if take_profit_1 <= optimal_entry:
            #     take_profit_1 = optimal_entry * 1.03
            # if take_profit_2 <= take_profit_1:
            #     take_profit_2 = optimal_entry * 1.07
            ...
            # Score-based confidence
            base_confidence += tech_score * 4
            base_confidence += trend_score * 4
            base_confidence += stoch_score * 4
            base_confidence += structure_score * 3 # Same weight
            ...
            confidence = min(90, base_confidence)  # TUNED: Lower cap (was 90)

            # Return the BUY signal dictionary
            return {
                'symbol': symbol_data['symbol'],
                'side': 'Buy',
                'order_type': order_type,
    ...
    def create_sell_signal(self, symbol_data: Dict, current_price: float, latest: pd.Series,
                        volume_entry: Dict, confluence_zones: List[Dict],
                        tech_score: int, trend_score: int, stoch_score: int, structure_score: int) -> Dict:
        """TUNED: Create SELL signal using market structure for stop loss and take profit"""
        try:
            # Entry price logic for SELL
            entry_candidates = [current_price]
            ...
            resistance_zones = [zone for zone in confluence_zones
                              if zone['zone_type'] == 'resistance' and 
                              zone['price'] > current_price * 0.98 and
                              zone['price'] < current_price * 1.05]
            ...
            # stop_loss = max([zone['price'] for zone in confluence_zones if zone['zone_type'] == 'resistance' and zone['price'] > optimal_entry], default=optimal_entry * 1.03)
            # Find nearest support below entry for take profit
            take_profit_1 = max([zone['price'] for zone in confluence_zones if zone['zone_type'] == 'support' and zone['price'] < optimal_entry])
            # Optionally, set a further TP2 at next support or a fixed multiple
            take_profit_2 = max([zone['price'] for zone in confluence_zones if zone['zone_type'] == 'support' and zone['price'] < take_profit_1])
            ...
            if take_profit_2 >= take_profit_1:
                take_profit_2 = 0
            # if stop_loss <= optimal_entry:
            #     stop_loss = optimal_entry * 1.03
            # if take_profit_1 >= optimal_entry:
            #     take_profit_1 = optimal_entry * 0.97
            # if take_profit_2 >= take_profit_1:
            #     take_profit_2 = optimal_entry * 0.93
            ...
            # Return the SELL signal dictionary
            return {
                'symbol': symbol_data['symbol'],
                'side': 'Sell',
                'order_type': order_type,
