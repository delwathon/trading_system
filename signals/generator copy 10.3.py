...
VERSION 10.1 - MERGED WITH MTF VALIDATOR - FIXED STOP LOSS ISSUES
...
✅ Merged MTF Validator for streamlined operation
...
class StrictMTFValidator:
    ...
    def __init__(self, confirmation_timeframes: List[str]):
        ...
        self.min_tp1_reachability = 0.85  # 70% confidence for TP1
...
class AdaptiveSignalConfig:
    ...
    min_stop_distance_pct: float = 4.0  # 1% minimum stop (was 0.05%)
    max_stop_distance_pct: float = 10.0  # 5% maximum stop (was 0.10%)
    ...
    def adapt_to_market(self, volatility: float, volume: float, trend_strength: float, 
        ...
        if self.current_market_condition == MarketCondition.EXTREME:
            ...
            self.min_stop_distance_pct = 5.0  # Much wider (was 0.08)
            self.max_stop_distance_pct = 10.0  # Much wider
            ...
            self.min_stop_distance_pct = 5.0  # Wider (was 0.06)
            self.max_stop_distance_pct = 10.0 # Wider
            ...
            self.min_stop_distance_pct = 5.0  # Standard (was 0.05)
            self.max_stop_distance_pct = 10.0  # Standard
            ...
            self.min_stop_distance_pct = 5.0  # Tighter but reasonable (was 0.04)
            self.max_stop_distance_pct = 10.0  # Still reasonable
...
class SignalGenerator:
    ...
    With FIXED stop losses for crypto trading and integrated MTF validator
    """
    ...
    def __init__(self, config: EnhancedSystemConfig, exchange_manager=None):
        ...
        self.logger.info("✅ Enhanced Signal Generator v10.1 initialized (MERGED + FIXED STOP LOSSES)")
        ...
        self.logger.info(f"   MTF Validator: INTEGRATED")
    ...
    def _generate_long_signal(self, symbol_data: Dict, latest: pd.Series,
        ...
        try:
            ...
            order_type = 'limit' if entry_distance_pct > 0.008 else 'market'
...
__version__ = "10.1.0-merged"
__features__ = [
    "✅ MERGED: MTF Validator integrated into main generator",
    ...
    "✅ ENHANCED: Strict MTF validation prevents counter-trend signals",
    "✅ ENHANCED: TP1 reachability assessment for realistic targets",
...
from config.config import EnhancedSystemConfig
from your_exchange import ExchangeManager
...
# Analyze symbol
signal = generator.analyze_symbol_comprehensive(
    df=your_dataframe,
    symbol_data=your_symbol_data,
    volume_entry=your_volume_entry,
    fibonacci_data=your_fibonacci_data,
    confluence_zones=your_confluence_zones,
...
if signal:
    ...
    print(f"MTF Status: {signal['mtf_status']}")
"""