...
class SignalGenerator:
    ...
    def _generate_traditional_signal(self, df: pd.DataFrame, symbol_data: Dict,
        ...
        """Generate signal with DUAL MODE based on relaxed_mode setting - FIXED REQUIREMENTS"""
        try:
            latest = df.iloc[-1]
            ...
            if self.relaxed_mode:
                # ===== RELAXED MODE: SCORING SYSTEM - TIGHTENED =====
                long_score = 0
                short_score = 0
                long_factors = []
                short_factors = []
                ...
                # Volume scoring (weight: 1 point) - TIGHTENED
                ...
                # FIXED: Increased minimum score needed
                min_score_needed = 4.5 * self.signal_config.adaptation_factor  # Was 3.0
                ...
                # ===== STRICT MODE: REQUIRE ALL CONDITIONS - ENHANCED =====
                long_conditions = [
                ...
                if all(long_conditions):
                    self.logger.debug(f"✅ All LONG conditions met for {symbol_data['symbol']}")
                    return self._generate_long_signal(
                        symbol_data, latest, mtf_context, volume_entry,
                        confluence_zones, df, intel, use_mtf=False
                    )
                elif all(short_conditions):
                    self.logger.debug(f"✅ All SHORT conditions met for {symbol_data['symbol']}")
                    return self._generate_short_signal(
                        symbol_data, latest, mtf_context, volume_entry,
                        confluence_zones, df, intel, use_mtf=False
                    )
