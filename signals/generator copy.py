...
class SignalGenerator:
    ...
    def create_context_aware_buy_signal(self, symbol_data: Dict, current_price: float, latest: pd.Series,
        ...
        try:
            ...
            reward_amount = take_profit_1 - optimal_entry if self.config.default_tp_level == 'take_profit_1' else take_profit_2 - optimal_entry                
    ...
    def create_context_aware_sell_signal(self, symbol_data: Dict, current_price: float, latest: pd.Series,
        ...
        try:
            ...
            reward_amount = optimal_entry - take_profit_1 if self.config.default_tp_level == 'take_profit_1' else optimal_entry - take_profit_2
            risk_reward_ratio = reward_amount / risk_amount if risk_amount > 0 else 0
    ...
    def rank_opportunities_with_mtf(self, signals: List[Dict], dfs: Optional[Dict[str, pd.DataFrame]] = None) -> List[Dict]:
        ...
        try:
            ...
            final_result = filtered_opportunities[:self.config.charts_per_batch]

            if not self.config.use_old_analysis:
                #sort final result by optimization_applied = True
                final_result = sorted(final_result, key=lambda x: x.get('optimization_applied', False), reverse=True)
        ...
        try:
            ...
            if signal['stop_loss'] != 0:
                if signal['side'] == 'buy':
                    ...
                    reward_amount = abs(signal['take_profit_1'] - signal['entry_price']) if self.config.default_tp_level == 'take_profit_1' else abs(signal['take_profit_2'] - signal['entry_price'])
                else:
                    risk_amount = abs(signal['stop_loss'] - signal['entry_price'])
                    reward_amount = abs(signal['entry_price'] - signal['take_profit_1']) if self.config.default_tp_level == 'take_profit_1' else abs(signal['entry_price'] - signal['take_profit_2'])
                risk_reward_ratio = reward_amount / risk_amount if risk_amount > 0 else 0
            else:
                risk_reward_ratio = 0
